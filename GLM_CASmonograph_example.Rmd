...
abstract: "This notebooks demonstrates how to fit a simple reserving GLM using R drawn from a 2015 CAS monograph on stochastic reserving."
...
This notebook provides a worked example in R of fitting a GLM to some non-life claims reserving data. 
The example and data are drawn from the CAS Monograph [Stochastic Loss Reserving using Generalized Linear Models](https://www.casact.org/pubs/monographs/index.cfm?fa=taylor-monograph03).
Here we follow through the application of the cross-classified model from that monograph to the data (Chapter 3), and follow through with the additonal work to firstly simplify the model and secondly to imporve the model fit through the use of some interaction terms (Chapter 7).


If you want to access the underlying Rnotebook directly, you may access it at: 

* [GLM_CASmonograph_example.Rmd](https://github.com/grainnemcguire/grainnemcguire.github.io/blob/master/GLM_CASmonograph_example.Rmd). 
* The data used is available in this [csv file](https://github.com/grainnemcguire/grainnemcguire.github.io/blob/master/glms_meyershi.csv)
...
* [magrittr](https://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html) Use the `%>%` pipe to reduce nested parentheses
...
msdata <- data.table::fread(here::here("data/glms_meyershi.csv"))
...
glm_fit1$coeff_table <- data.table(parameter = names(glm_fit1$coefficients), coeff_glm_fit1 = glm_fit1$coefficients)
...
```{r}
msdata[, residuals1 := glm_fit1$residuals
...
```{r}
msdata[, residuals4 := glm_fit4$residuals
...
msdata[, residuals5 := glm_fit5$residuals
...
# Feedback
Please send any feedback or error corrections to me at [grainnemcguire@yahoo.com](mailto:grainnemcguire@yahoo.com)


# Session information

Here are details of my R session.
